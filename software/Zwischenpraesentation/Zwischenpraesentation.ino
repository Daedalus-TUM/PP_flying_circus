...
#define periodendauer 100  //Periodendauer für timer in ms
...
PID hoehePID(&istH, &geschwindigkeitH, &sollH, 5, 0.6, 13, DIRECT);    //funktioniert ganz gut
...
void Periode() {
  ...
  Serial.println(sollH);
  Serial.println(istH);
  Serial.println();
  
  //Filter für Sprünge in der Höhenregelung
  //if(reading < (hoehe[idummy] - 15)) offsetH = hoehe[idummy]-reading;
  //else if(reading > (hoehe[idummy] + 15)) offsetH = hoehe[idummy]-reading;
