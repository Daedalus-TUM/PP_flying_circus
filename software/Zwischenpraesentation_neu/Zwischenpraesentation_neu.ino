...
double sollH = 100;     //Sollhoehe für Hoehenregelung
...
byte hoehe[5] = {0};  //Speicher für die aktuellsten 5 Messwerte aus der Hoehenmessung vom Ultraschallsensor
...
PID hoehePID(&istH, &geschwindigkeitH, &sprungH, 5, 0.6, 13, DIRECT);    //funktioniert ganz gut
...
void NRF() {
  if(Mirf.dataReady()) {
    ...
    if((receiving[0] == 0) && (receiving[1] == 30)) {
      sollH = receiving[6];    //Uebernehmen der Sollhoehe
  ...
  if (handler == 0) {
    ...
    if(2 <= Wire.available())   // if two bytes were received
    {
      ...
      if((reading > 0) && (reading < 300)) {
        ...
        if(reading < (istH - 10)) sprungH = sollH - (istH - reading);
        else if(reading > (istH + 10)) sprungH = sollH + (reading - istH);
        idummy++;
    ...
    motor(motor_H, 0, geschwindigkeitH);
...
void setup() {
  ...
  hoehePID.SetOutputLimits(10, 140);    //funktioniert ganz gut
  hoehePID.SetMode(AUTOMATIC);  //aktiviert PID-Regler
  hoehePID.SetSampleTime(200);  //Sample Time des PID auf 100ms
